...
add_action('edd_payment_receipt_after', 'display_novalnet_payment_details');
...
{
    global $wpdb;
    if ('novalnet_cashpayment' === get_novalnet_payment($post->ID)) {
...
function novalnet_success($response, $update_payment, $trial_product)
{
    ...
    if (in_array($payment_gateway, $invoice_payments, true) || ('novalnet_paypal' === $payment_gateway && ('90' === $response['tid_status'] || '85' === $response['tid_status'])) || ('novalnet_przelewy24' === $payment_gateway && '86' === $response['tid_status']) || ('novalnet_cashpayment' === $payment_gateway)) {
    ...
    if (in_array($response['tid_status'], array('91', '99', '98', '85'), true)) {
        ...
        $final_order_status = !empty($edd_options[$payment_gateway . '_order_completion_status'])
            ...
            : ((in_array($payment_gateway, array('novalnet_cashpayment', 'novalnet_prepayment', 'novalnet_invoice'))) ? 'processing' : 'pending');
    ...
    if ('novalnet_cashpayment' === $payment_gateway) {
...
/**
 * Process to display Novalnet transaction comments.
 *
 * @since 2.2.1
 * @param object $payment Get relevant order object.
 * @return mixed
 */
function display_novalnet_payment_details($payment)
{
    if ($payment->id != null && novalnet_check_string($payment->gateway)) {
        $transaction_comments = novalnet_get_order_meta($payment->id, 'novalnet_transaction_comments');
        if (empty($transaction_comments)) {
            $novalnet_note = get_post($payment);
        ...
        if (!empty($transaction_comments)) {
            ...
            <div class="edd-blocks__row edd-blocks-receipt__row-item">
                <div class="edd-blocks__row-label" style="float=left; font-weight:400">
                    <b><?php esc_html_e('Transaction Comments', 'edd-novalnet'); ?></b>:
                    <?php echo wp_kses_post(wpautop(wptexturize($transaction_comments))); ?>
                </div>
            </div>
            <?php
        }
