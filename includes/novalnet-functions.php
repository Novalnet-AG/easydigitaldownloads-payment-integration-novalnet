...
function novalnet_barzahlen_scripts( $post ) {
    global $wpdb;
    if ( 'novalnet_cashpayment' === get_post_meta( $post->ID, '_edd_payment_gateway', true ) ) {
        $transaction_id = get_post_meta( $post->ID, '_nn_order_tid', true );
                ...
                if ( ! empty( $purchase_val['options']['recurring'] ) && ( empty( $purchase_val['options']['recurring']['trial_period'] ) && 1 === $purchase_val['options']['recurring']['times'] ) ) {
                    ...
                    if ( ! empty( $purchase_val['options']['recurring'] ) ) {
                        ...
                        if ( ! empty( $recurring['trial_period'] ) ) {
							...
							if ( 'yes' === $edd_options['prices_include_tax'] ) {
								$config_data['amount'] = $config_data['amount'] + $purchase_data['tax'];
							} else {
...
function novalnet_get_customer_data( $purchase_data ) {
    ...
    $customer_data = array_map( 'trim',
        array(
            'gender'           => 'u',
            'customer_no'      => $purchase_data['user_info']['id'] > 0 ? $purchase_data['user_info']['id'] : 'guest',
            'first_name'       => $name['0'],
            'last_name'        => $name['1'],
 ...
 */
function novalnet_check_response( $response, $update_payment = false ) {
    ...
    if ( isset( $response['status'] ) && '100' === $response['status'] ) {
        ...
        novalnet_success( $response, $update_payment );
    } else {
 ...
 */
function novalnet_success( $response, $update_payment ) {
    ...
    if ( $update_payment ) {
        ...
        $exist_comments = $wpdb->get_var( $wpdb->prepare( "SELECT post_excerpt FROM {$wpdb->posts} where ID =%s", $response['order_no'] ) );
        ...
        // Update Novalnet Transaction comments into shop database.
        wp_update_post(
            array(
                'ID'           => $response['order_no'],
                'post_excerpt' => $exist_comments . $comments,
            )
        );
    ...
    update_post_meta( $response['order_no'], '_edd_payment_gateway', $payment_gateways['gateway'] );
    update_post_meta( $response['order_no'], '_nn_order_tid', $response['tid'] );
    ...
    if ( in_array( $payment_gateways['gateway'], $invoice_payments, true ) || ( 'novalnet_paypal' === $payment_gateways['gateway'] && ( '90' === $response['tid_status'] || '85' === $response['tid_status'] ) ) || ( 'novalnet_przelewy24' === $payment_gateways['gateway'] && '86' === $response['tid_status'] ) || ( 'novalnet_cashpayment' === $payment_gateways['gateway'] ) ) {
        update_post_meta( $response['order_no'], '_nn_callback_amount', 0 );
    } else {
        // Set the purchase to complete.
        update_post_meta( $response['order_no'], '_nn_callback_amount', get_post_meta( $response['order_no'], '_edd_payment_total', true ) * 100 );
    }
    ...
    if ( in_array( $payment_gateways['gateway'], $invoice_payments, true ) ) {
...
function update_transaction_details( $response, $payment_gateway ) {
    ...
    $wpdb->insert(
        ...
        array(
            ...
            'payment_id'      => ( isset( $response['key'] ) && !empty( $response['key'] ) ) ? $response['key'] : $response['payment_id'],
...
function novalnet_transaction_detail_checkout( $post_comments ) {
    ...
    if ( novalnet_check_string( $payment_name ) ) {
        echo wpautop( edd_get_gateway_checkout_label( $payment_name ) . $nn_post_id->post_excerpt );
    }
}
...
function remove_novalnet_payment( $gateway_list ) {
    ...
    if ( ! is_admin() && ( empty( $edd_options['novalnet_public_key'] ) || empty( $edd_options['novalnet_tariff_id'] ) || empty( $edd_options['novalnet_client_key'] ) ) ) {
    ...
    foreach ( (array) $cart_details as $cart_val ) {
        if ( ! empty( $cart_val ['item_number']['options']['recurring'] ) ) {
            $recurring = true;
            break;
        }
    }
...
function novalnet_backend_comments( $comments ) {
    foreach ( $comments as $value ) {
        if ( novalnet_check_string( get_post_meta( $value->comment_post_ID, '_edd_payment_gateway', true ) ) ) {
...
function novalnet_sale_notification( $mail_body, $payment_id ) {
    if ( ! is_admin() && novalnet_check_string( get_post_meta( $payment_id, '_edd_payment_gateway', true ) ) ) {
...
function novalnet_get_invoice_comments( $response, $payment_gateways, $amount, $novalnet_comments, $cond = true, $callback = false ) {
    ...
    if ( isset( $response['tid_status'] ) && '75' === $response['tid_status'] && '41' === $response['key'] ) {
        ...
        if ( '100' === $response['tid_status'] ) {
            $novalnet_comments .= __( 'Due date: ', 'edd-novalnet' ) . date_i18n( get_option( 'date_format' ), strtotime( $response['due_date'] ) ) . PHP_EOL;
        }
...
function cashpayment_order_comments( $response ) {
    ...
    $comments = PHP_EOL;
    if ( $response['cashpayment_due_date'] ) {
        $comments .= __( 'Slip expiry date', 'edd-novalnet' ) . ': ' . $response['cashpayment_due_date'];
    }
 ...
 */
function novalnet_guarantee_payment_validation( $user_data, $payment_name ) {
        ...
        if ( isset( $user_data['downloads'] ) && !empty( $user_data['downloads'] ) ) {
           foreach ( $user_data['downloads'] as $purchase_key => $purchase_val ) {
        ...
        if ( '1' === $edd_options[ $payment_name . '_guarantee_enable' ] ) {
...
function get_novalnet_payment( $payment_id ) {
    $payment_name = get_post_meta( $payment_id, '_edd_payment_gateway', true);
    return $payment_name;
}
