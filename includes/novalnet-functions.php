...
// Add transaction comments to admin and customer emails
add_filter('edd_sale_notification', 'novalnet_add_transaction_comments_to_email', 10, 2);
add_filter('edd_purchase_receipt', 'novalnet_add_transaction_comments_to_email', 10, 2);

// Add transaction history to frontend order history page
add_action('edd_payment_receipt_after_table', 'novalnet_display_transaction_history');
    ...
    switch ($data['edd-api']) {
        ...
        case 'novalnet_update_payment_method':
            if (in_array($data['edd-api'], array('novalnet_redirect_response', 'products'), true)) {
                $order_data = !empty(EDD()->session->get('edd_purchase')) ? EDD()->session->get('edd_purchase') : edd_get_order($data['order_no']);
...
function novalnet_server_addr($type = 'REMOTE_ADDR')
{
    ...
    if ('SERVER_ADDR' === $type) {
        if (empty($_SERVER['SERVER_ADDR'])) {
            ...
            $ip_address = isset($_SERVER['SERVER_NAME']) ? gethostbyname(sanitize_text_field(wp_unslash($_SERVER['SERVER_NAME']))) : '';
        } else {
...
{
    return (!empty($string) && false !== strpos($string, $data));
}
...
{
        $payment_mode   =  !empty($_REQUEST['payment-mode']) ? sanitize_text_field(wp_unslash($_REQUEST['payment-mode'])) : '';
        $edd_gateway    =  !empty($_REQUEST['edd-gateway']) ? sanitize_text_field(wp_unslash($_REQUEST['edd-gateway'])) : '' ;
    
        if (((!empty($payment_mode) && novalnet_check_string($payment_mode)) && (!empty($edd_gateway) && novalnet_check_string($edd_gateway))) || (!empty($_REQUEST['tid']) && (!empty($_REQUEST['payment_method']) && in_array($_REQUEST['payment_method'], array('novalnet_paypal', 'novalnet_cc'), true)))) { // Input var okay.
...
function novalnet_success($response, $update_payment, $trial_product)
{
    ...
    if ($update_payment) {
        ...
        add_concat_comments($response['order_no'], $comments);
        $wpdb->update(
    ...
    $payment_gateway = (!empty(EDD()->session->get('edd_purchase')) && isset(EDD()->session->get('edd_purchase')['gateway']) ? EDD()->session->get('edd_purchase')['gateway'] : edd_get_payment_gateway($response['order_no']));
    novalnet_update_order_meta($response['order_no'], '_edd_payment_gateway', $payment_gateway);
    ...
    $amount = (isset($payment_gateway) && in_array($payment_gateway, array('novalnet_invoice', 'novalnet_prepayment', 'novalnet_sepa'), true)) ? str_replace(',', '', sprintf('%0.2f', $response['amount'])) * 100 : str_replace(',', '', sprintf('%0.2f', $response['amount']));
    if (in_array($payment_gateway, $invoice_payments, true) || ('novalnet_paypal' === $payment_gateway && ('90' === $response['tid_status'] || '85' === $response['tid_status'])) || ('novalnet_przelewy24' === $payment_gateway && '86' === $response['tid_status'])) {
    ...
    if (in_array($response['tid_status'], array('91', '99', '98', '85'), true)) {
        $final_order_status = !empty($edd_options['novalnet_onhold_success_status']) ? $edd_options['novalnet_onhold_success_status'] : 'pending';
    } elseif (in_array($response['tid_status'], array('75', '90', '86'), true)) {
        $final_order_status = !empty($edd_options[$payment_gateway . '_order_pending_status']) ? $edd_options[$payment_gateway . '_order_pending_status'] : 'pending';
    } else {
        $final_order_status = !empty($edd_options[$payment_gateway . '_order_completion_status'])
            ? $edd_options[$payment_gateway . '_order_completion_status']
            : ((in_array($payment_gateway, array('novalnet_prepayment', 'novalnet_invoice'))) ? 'processing' : 'pending');
        if (!empty($response['key']) && '41' === $response['key']) {
            $final_order_status = $edd_options[$payment_gateway . '_order_callback_status'];
        }
    }
    $test_mode = (int) (edd_is_test_mode() || !empty($edd_options[$payment_gateway . '_test_mode']) || !empty($response['test_mode']));
    $novalnet_comments = '';
    $novalnet_comments .= novalnet_form_transaction_details($response, $test_mode);
    if (in_array($payment_gateway, $invoice_payments, true) && $trial_product == '0') {
        $novalnet_comments = novalnet_get_invoice_comments($response, $amount, $novalnet_comments);
    }
    update_transaction_details($response, $payment_gateway);
    ...
    edd_set_payment_transaction_id($response['order_no'], $response['tid']);
    edd_update_payment_status($response['order_no'], $final_order_status);
    // Update Novalnet Transaction details into payment note.
    $nn_comments = novalnet_get_order_meta($response['order_no'], 'novalnet_transaction_comments');
    edd_insert_payment_note($response['order_no'], $novalnet_comments);

    if (isset($payment_gateway) && in_array($payment_gateway, array('novalnet_invoice', 'novalnet_prepayment', "novalnet_invoice_guarantee"), true) && $response["tid_status"] != "75" && $trial_product == '0') {
        $novalnet_comments .= PHP_EOL . get_epc_qr($response);
    }
    novalnet_update_order_meta($response['order_no'], 'novalnet_transaction_comments', $nn_comments . PHP_EOL . $novalnet_comments);
...
function update_transaction_details($response, $payment_gateway)
{
    ...
    $wpdb->insert(
        ...
        array(
            ...
            'payment_id' => !empty($response['key']) ? $response['key'] : (!empty($response['payment_id']) ? $response['payment_id'] : (is_array($payment_key = get_payment_key($payment_gateway)) && isset($payment_key['key']) ? $payment_key['key'] : '')),
...
function novalnet_transaction_failure($response, $hash = false, $update_payment = false)
{
    ...
    global $edd_options, $wpdb;
    $nn_tid = $wpdb->get_row($wpdb->prepare("SELECT tid FROM {$wpdb->prefix}novalnet_transaction_detail WHERE order_no=%s ORDER BY id DESC", $response['order_no']), ARRAY_A);
    if (!empty($nn_tid['tid']) && !$update_payment) {
        return;
    }
    ...
    edd_send_back_to_checkout('?payment-mode=' . $payment_gateway);
}
...
function edd_novalnet_refund($order_id, $refund_id, $all_refunded)
{
    ...
    if ($response['status'] == '100') { // Refund success
        $old_refund_amount = $wpdb->get_var($wpdb->prepare("SELECT refund_amount FROM {$wpdb->prefix}novalnet_transaction_detail WHERE tid=%s", $params['tid']));
        $refund_amount = $old_refund_amount + abs($refund->total) * 100;
        $note_object_ids = array($order->id);
        if ($refund instanceof Order) {
            $note_object_ids[] = $refund->id;
        }
        if (!empty($response['tid'])) {
            /* translators: %1$s is the parent tid, %2$s is the amount, %3$s is the new tid */
            ...
            /* translators: %1$s is the parent tid, %2$s is the amount */
        ...
        add_concat_comments($order->id, $note_message);
...
?>
    <div class="edd-form-group edd-novalnet-refund-transaction">
        <div class="edd-form-group__control">
            <input type="checkbox" id="edd-novalnet-refund" name="edd-novalnet-refund" class="edd-form-group__input"
                value="1">
...
function edd_novalnet_cancel_subscription($order)
{
    ...
    if ($response['status'] == '100') {
        /* translators: %s tid */
        $note_message = sprintf(__('Subscription has been canceled for the TID: %s', 'edd-novalnet'), $params['tid']);
        edd_add_note(array(
            'object_id' => $order->id,
            'object_type' => 'order',
            'user_id' => is_admin() ? get_current_user_id() : 0,
        ...
        /* translators: %s subscription cancel message */
        $note_message = sprintf(__('Subscription cancelation failed for the order due to: %s', 'edd-novalnet'), $response['status_desc']);
        edd_add_note(array(
            'object_id' => $order->id,
            'object_type' => 'order',
            'user_id' => is_admin() ? get_current_user_id() : 0,
...
function novalnet_get_redirect_response($update_payment = false)
{
    ...
    if (empty($order_ref) || empty($order_ref->payment_type) || (!empty($order_ref) && $update_payment)) {
...
function novalnet_display_payment_logo($payment_method, $payment_method_name)
{
    ...
    if ($icon_html == 'yes') {
        if ('novalnet_cc' !== $payment_method) { ?>
            <img class="payment-icon" title="<?php echo esc_html($payment_method_name); ?>"
                alt="<?php echo esc_html($payment_method_name); ?>" style="float:right;"
                src="<?php echo esc_html(NOVALNET_PLUGIN_URL . 'assets/images/' . $payment_method . '.png'); ?>">
            <?php
        }
        if ('novalnet_cc' === $payment_method) {
            ...
            ?>
                <img class="payment-icon" title="<?php echo esc_html($payment_method_name); ?>"
                    alt="<?php echo esc_html($payment_method_name); ?>" style="float: right;"
                    src="<?php echo esc_html(NOVALNET_PLUGIN_URL) . 'assets/images/' . esc_html($value) . '.png'; ?>">
<?php
            }
        }
    }
...
function remove_novalnet_payment($gateway_list)
{
    ...
    if (!is_admin() && !empty(EDD()->cart->contents)) {
        ...
        if ($recurring && isset($edd_options['novalnet_subs_payments'])) {
            ...
            if (isset($edd_options['default_gateway']) && novalnet_check_string($edd_options['default_gateway']) && !empty($edd_options['novalnet_subs_payments']) &&  !in_array($edd_options['default_gateway'], $edd_options['novalnet_subs_payments'], true) && !empty($edd_options['novalnet_subs_enable_option']) && '1' === $edd_options['novalnet_subs_enable_option']) {
            ...
            } else if (isset($edd_options['default_gateway']) && novalnet_check_string($edd_options['default_gateway']) && !empty($gateway_list) && !in_array($edd_options['default_gateway'], array_keys($gateway_list), true)) {
...
function novalnet_backend_comments($comments)
{
    foreach ($comments as $value) {
        if (novalnet_check_string(get_novalnet_payment($value->comment_post_ID))) {
...
{
    if (!is_admin() && novalnet_check_string(get_novalnet_payment($payment_id))) {
 ...
 * @return array
 */
function novalnet_get_invoice_comments($response, $amount, $novalnet_comments, $cond = true, $callback = false)
{
    global $edd_options, $wpdb;
    ...
    if (isset($response['tid_status']) && '75' === $response['tid_status'] && '41' === $response['key']) {
        ...
        $amount = edd_currency_filter( edd_format_amount( $response['amount']));
        if ($callback) {
            $amount = edd_currency_filter(edd_format_amount($response['amount'] / 100));
        }
        /* translators: %1$s amount */
        $novalnet_comments_due = sprintf(__('Please transfer the amount %1$s to the following account.', 'edd-novalnet'), $amount) . PHP_EOL;
        if ('100' === $response['tid_status']) {
            ...
            $novalnet_comments_due = sprintf(__('Please transfer the amount %1$s to the following account on or before %2$s', 'edd-novalnet'), $amount, $due_date) . PHP_EOL;
        }
        $novalnet_comments .= $novalnet_comments_due;
        ...
        $novalnet_comments .= ' Bank: ' . $response['invoice_bankname'] . PHP_EOL;
        $novalnet_comments .= 'Place: ' . trim(isset($response['invoice_bankplace']) ? $response['invoice_bankplace'] : '') . PHP_EOL;
        $increment = 1;
        if ($cond) {
...
function get_novalnet_payment($payment_id)
{
    ...
    if (empty($payment_name)) {
        $payment_name = edd_get_payment_gateway($payment_id);
    }
    return $payment_name;
}
...
/**
 * Add Novalnet transaction comments to purchase receipt emails
 *
 * @param string $message Email message content
 * @param int $payment_id Payment ID
 * @return string Modified email message
 */
function novalnet_add_transaction_comments_to_email($message, $payment_id)
{
    // Check if it's a Novalnet payment
    if (!novalnet_check_string(get_novalnet_payment($payment_id))) {
        return $message;
    }

    // Get transaction comments
    $transaction_comments = novalnet_get_order_meta($payment_id, 'novalnet_transaction_comments');

    // If still empty, try getting from payment notes
    if (empty($transaction_comments)) {
        $payment_notes = edd_get_payment_notes($payment_id);
        foreach ($payment_notes as $note) {
            if (strpos($note->comment_content, 'Novalnet') !== false) {
                $transaction_comments .= $note->comment_content . PHP_EOL;
            }
        }
    }

    // Add comments to email if found
    if (!empty($transaction_comments)) {
        $message .= PHP_EOL . PHP_EOL . __('Transaction Details :', 'edd-novalnet');
        $message .= PHP_EOL . strip_tags($transaction_comments, '<img>');
    }
...
/**
 * Process to display EPC QR to Novalnet transaction comments.
 *
 * @since 2.2.1
 * @param object $request_param Get relevant order object.
 * @return string
 */
function get_epc_qr($request_param)
{
    $qrimage = $request_param['invoice_qr_image'];
    $novalnet_comments = PHP_EOL . __('Alternatively, you can use the QR code below for your convenience. Please scan it with your banking app to complete the payment.', 'edd-novalnet') . PHP_EOL;
    $novalnet_comments .= '<img src=' . $qrimage . ' alt="QR Code">';
    return $novalnet_comments;
}

/**
 * Display Novalnet transaction history on frontend order history page
 *
 * @param object $payment Payment object
 */
function novalnet_display_transaction_history($payment)
{
    if (!novalnet_check_string($payment->gateway)) {
        return;
    }

    // Get all payment notes for this order
    $transaction_comments = novalnet_get_order_meta($payment->id, 'novalnet_transaction_comments');
    if (empty($transaction_comments)) {
        $novalnet_note = get_post($payment->ID);
        $transaction_comments = wpautop($novalnet_note->post_excerpt);
    }

    if (empty($transaction_comments)) {
        $transaction_comments = edd_get_payment_notes($payment->id, 'novalnet');
        $novalnet_command = '';
        foreach ($transaction_comments as $transaction_comment) {
            if (preg_match('/Novalnet/', $transaction_comment->comment_content)) {
                $novalnet_command .= $transaction_comment->comment_content . PHP_EOL;
            }
        }
        $transaction_comments = $novalnet_command;
    }
    ...
    if (!empty($transaction_comments)) {
        // Allow <img> tags and convert newlines to <br>
        $safe_content = wp_kses($transaction_comments, $allowed_tags);
        echo esc_html(__('Transaction Details :', 'edd-novalnet')) . PHP_EOL . nl2br($safe_content);
    }
}

/**
 * Concat Novalnet transaction history on frontend order history page
 *
 * @param object $payment Payment object
 */
function add_concat_comments($order_no, $message)
{
    $old_comment = novalnet_get_order_meta($order_no, 'novalnet_transaction_comments');
    novalnet_update_order_meta($order_no, 'novalnet_transaction_comments', $old_comment . PHP_EOL . $message);
}
