...
class Novalnet_Global_Config {
    ...
    public static function global_admin_script() {
        ...
        // Enqueue script.
        wp_enqueue_script( 'novalnet-admin-script', NOVALNET_PLUGIN_URL . 'assets/js/novalnet-admin.min.js', '', NOVALNET_VERSION, true);

        $integrity_hash = 'sha384-he7v4LZ1bxB/nuJVy/DjDx0qo8M4Z1+AlNHqgfFn1C15EADiEvD40T14i+s9yT/f';
        add_filter('script_loader_tag', function ($tag, $handle) use ($integrity_hash) {
            if ('novalnet-admin-script' === $handle) {
                return str_replace(
                    ' src=',
     ...
     */
    public static function register_novalnet_global_settings( $gateway_settings ) {
        ...
        $novalnet_global = array(
            array(
                ...
                'name' => '<strong><font color="#1874CD">' . __( 'Novalnet API Configuration', 'edd-novalnet' ) . '</font> </strong> <p style="width:550%;">' . sprintf( __( 'Please read the Installation Guide before you start and login to the <a href="%s" target="blank">Novalnet Admin Portal</a> using your merchant account. To get a merchant account, mail to <a href="mailto:sales@novalnet.de">sales@novalnet.de</a> or call +49 (089) 923068320.', 'edd-novalnet' ), $admin_url) . '</p>',
            ...
            array(
                ...
                'tooltip_desc'  => __( 'Your product activation key is a unique token for merchant authentication and payment processing.', 'edd-novalnet' ),
                /* translators: %s: admin URL */
                'desc'              => '<br/>' . sprintf( __( "Get your Product activation key from the <a href='%s' target='blank'>Novalnet Admin Portal</a>: Projects > Choose your project > API credentials > API Signature (Product activation key)","edd-novalnet"), $admin_url ),
            ),
            array(
            ...
            array(
                ...
                /* translators: %s url */
                'desc'  => '<br/>' . sprintf(__( 'You must add the following webhook endpoint to your <a href="%s" target="blank">Novalnet Admin Portal</a> . This will allow you to receive notifications about the transaction status.', 'edd-novalnet' ), $admin_url ),
                'allow_blank'   => false,
            ),
        );
