...
class Novalnet_SEPA {
    ...
    public function novalnet_sepa_script_functions() {
        ...
        wp_enqueue_script( 'edd-novalnet-sepa-script', NOVALNET_PLUGIN_URL . 'assets/js/novalnet-sepa.min.js', array( 'jquery' ), NOVALNET_VERSION, true );
        wp_enqueue_script( 'edd-novalnet-sepa-script-dob', NOVALNET_PLUGIN_URL . 'assets/js/novalnet.min.js', array( 'jquery' ), NOVALNET_VERSION, true );

        $integrity_hash = 'sha384-eRbPbe7xDR7N7Z8SCS1hHqfnxcVK0qJNmrPFxL/COeQ27kL3b9vlvkG42ouy7RJB';
        add_filter('script_loader_tag', function ($tag, $handle) use ($integrity_hash) {
            if ('edd-novalnet-sepa-script' === $handle) {
                return str_replace(
                    ' src=',
            ...
            if ('edd-novalnet-sepa-script-dob' === $handle) {
     ...
     */
    public static function register_novalnet_sepa_settings( $gateway_settings ) {
        ...
        $novalnet_sepa = array(
            ...
            array(
                ...
                'min'           => 3,
                'max'           => 14,
                'placeholder'          => 'must be between 3 and 14 days',
                'tooltip_title' => __( 'Payment due date (in days)', 'edd-novalnet' ),
                'tooltip_desc'  => __( 'Number of days after which the payment is debited (must be between 3 and 14 days).', 'edd-novalnet' ),
            ),
            array(
