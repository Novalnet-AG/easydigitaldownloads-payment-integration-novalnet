 ...
 *
 * This gateway is used for real time processing of Ideal Payment.
 *
...
/**
 * IDEAL payment processed
 */
class Novalnet_Ideal {

	/**
	 * Get all required action and filter to process iDEAL
	 */
	public function __construct() {

		add_action( 'update_option_novalnet_settings', array( $this, 'update_novalnet_settings' ) );
		add_filter( 'edd_payment_gateways', array( $this, 'register_novalnet_ideal' ), 1, 1 );
		add_action( 'edd_novalnet_ideal_cc_form', array( $this, 'display_form' ) );
		add_action( 'edd_gateway_novalnet_ideal', array( $this, 'novalnet_ideal_process_payment' ) );
		if ( is_admin() ) {
			add_filter( 'edd_settings_sections_gateways', array( $this, 'register_novalnet_ideal_gateway' ), 1, 1 );
			add_filter( 'edd_settings_gateways', array( $this, 'register_novalnet_ideal_settings' ), 1, 1 );
		}
	...
	public static function update_novalnet_settings() {
		// Update ideal configuraion fields.
		update_option( self::register_novalnet_ideal_settings() );
	}
	 ...
	 * @since  1.1.0
	 * @access public
	 * @param  array $gateway_sections Get ideal payment to append in default gateways.
	 * @return array $gateway_sections Returns ideal in along with gateway payments.
	 */
	public function register_novalnet_ideal_gateway( $gateway_sections ) {
		if ( edd_is_gateway_active( 'novalnet_ideal' ) ) {
			$gateway_sections['novalnet_ideal'] = __( 'Novalnet iDEAL', 'edd-novalnet' );
		}
		return $gateway_sections;
	}

	/**
	 * Register the gateway for ideal
	 *
	 * @access public
	 * @param array $gateways Allows payment details.
	 * @return array  $gateways Show ideal payment in front-end and back-end.
	 */
	public function register_novalnet_ideal( $gateways ) {
		$novalnet_ideal = array(
			'novalnet_ideal' => array(
				'admin_label'    => __( 'Novalnet iDEAL', 'edd-novalnet' ),
				'checkout_label' => __( 'iDEAL', 'edd-novalnet' ),
			),
		);
		$novalnet_ideal = apply_filters( 'edd_register_novalnet_ideal_gateway', $novalnet_ideal );
		$gateways       = array_merge( $gateways, $novalnet_ideal );
		return $gateways;
	}
	...
	public function display_form() {
		global $edd_options;
		$test_mode           = (int) ( edd_is_test_mode() || ! empty( $edd_options['novalnet_ideal_test_mode'] ) );
		$information         = isset( $edd_options['novalnet_ideal_information'] ) ? trim( $edd_options['novalnet_ideal_information'] ) : '';
		$payment_method_name = __( 'iDEAL', 'edd-novalnet' );
		?>
		<fieldset>
			<div class="edd-payment-icons">
				<?php
					echo esc_html( $payment_method_name );
					novalnet_display_payment_logo( 'novalnet_ideal', $payment_method_name );
				?>
			</div>
			<p>
				<label for="novalnet_ideal_payment_desc" >
					<span class="edd-description" style="font-size:90%;">
						<?php echo esc_html( __( 'You will be redirected to iDEAL. Please don’t close or refresh the browser until the payment is completed', 'edd-novalnet' ) ); ?>
					</span>
				</label>
			</p>
			<p>
				<label for="novalnet_ideal_information">
	 ...
	 */
	public function novalnet_ideal_process_payment( $purchase_data ) {
		
		$paygate_url = 'https://payport.novalnet.de/online_transfer_payport';
		// Get configuration data.
		$payment_data = novalnet_get_merchant_data( $purchase_data, 'novalnet_ideal' );
		// Get customer data.
	 ...
	 */
	public function register_novalnet_ideal_settings( $gateway_settings ) {
		$novalnet_ideal                     = array(
			array(
				'id'   => 'novalnet_ideal_settings',
				'name' => '<strong> <font color="#1874CD">' . __( 'Novalnet iDEAL', 'edd-novalnet' ) . '</font> </strong>',
		...
		$novalnet_ideal                     = apply_filters( 'edd_novalnet_ideal_settings', $novalnet_ideal );
		$gateway_settings['novalnet_ideal'] = $novalnet_ideal;
