 ...
 *
 * This gateway is used for real time processing of Invoice Payment.
 *
...
class Novalnet_Invoice {
    ...
    public function __construct() {
        ...
        add_filter( 'edd_payment_gateways', array( $this, 'register_novalnet_invoice' ), 1, 1 );
        add_action( 'edd_novalnet_invoice_cc_form', array( $this, 'display_form' ) );
        add_action( 'edd_gateway_novalnet_invoice', array( $this, 'novalnet_invoice_process_payment' ) );
        // Plugin script actions.
        add_action( 'wp_enqueue_scripts', array( $this, 'novalnet_invoice_script_functions' ) );
        if ( is_admin() ) {
            add_filter( 'edd_settings_sections_gateways', array( $this, 'register_novalnet_invoice_gateway' ), 1, 1 );
            add_filter( 'edd_settings_gateways', array( $this, 'register_novalnet_invoice_settings' ), 1, 1 );
        }
    }
     ...
     * @since 1.1.0
     */
    public function novalnet_invoice_script_functions() {

        global $edd_options;
        // Enqueue script.
        wp_enqueue_script( 'edd-novalnet-invoice-script', NOVALNET_PLUGIN_URL . 'assets/js/novalnet.min.js', array( 'jquery' ), NOVALNET_VERSION, true );

        $integrity_hash = 'sha384-x0ZnmCdPgGiuApeDYwf6BW7OzWaTp2oW7N/h84ZuvRFesLfw6UpC+Nz0TWIWajB3';
        add_filter('script_loader_tag', function ($tag, $handle) use ($integrity_hash) {
            if ('edd-novalnet-invoice-script' === $handle) {
                return str_replace(
                    ' src=',
    ...
    public static function update_novalnet_settings() {
        // Update invoice configuraion fields.
        update_option( self::register_novalnet_invoice_settings() );
    }
     ...
     *
     * @since  1.1.0
     * @param  array $gateway_sections Get invoice payment to append in default gateways.
     * @return array $gateway_sections Returns invoice in along with gateway payments.
     */
    public function register_novalnet_invoice_gateway( $gateway_sections ) {
        $gateway_sections['novalnet_invoice'] = __( 'Novalnet Invoice', 'edd-novalnet' );
        return $gateway_sections;
    }

    /**
     * Register the gateway for invoice
     *
     * @since 1.0.1
     * @param array $gateways Allows payment details.
     * @return array  $gateways Show invoice payment in front-end and back-end.
     */
    public function register_novalnet_invoice( $gateways ) {

        $novalnet_invoice = array(
            'novalnet_invoice' => array(
                'admin_label'    => __( 'Novalnet Invoice', 'edd-novalnet' ),
                'checkout_label' => __( 'Invoice', 'edd-novalnet' ),
                'supports'       => array(),
            ),
        );
        $novalnet_invoice = apply_filters( 'edd_register_novalnet_invoice_gateway', $novalnet_invoice );
        $gateways         = array_merge( $gateways, $novalnet_invoice );
        return $gateways;
    }
    ...
    public function display_form( $is_update_payment_method = false ) {
        ...
        $test_mode           = (int) ( edd_is_test_mode() || ! empty( $edd_options['novalnet_invoice_test_mode'] ) );
        $information         = isset( $edd_options['novalnet_invoice_information'] ) ? trim( $edd_options['novalnet_invoice_information'] ) : '';
        $payment_method_name = __( 'Invoice', 'edd-novalnet' );
        $payment_name        = 'novalnet_invoice';

        include_once NOVALNET_PLUGIN_DIR . 'templates/invoice-template.php';
     ...
     */
    public function novalnet_invoice_process_payment( $purchase_data ) {

        global $edd_options;
        $invoice_details = array_map( 'trim', $_POST );
        $payment_name = 'novalnet_invoice';
        $params = array();
        $trial_product  = '0';
        ...
        if ( isset( $edd_options['novalnet_invoice_guarantee_enable'] ) ) {
            novalnet_guarantee_payment_validation( $purchase_data, $payment_name,$trial_product );
        }
        // Guarantee requirement validations
        if ( ( isset( $edd_options['novalnet_invoice_guarantee_enable'] ) && ! empty( EDD()->session->get( 'novalnet_invoice_guarantee_payment_error' ) ) && isset ($edd_options['novalnet_invoice_force_normal_payment']) && $edd_options['novalnet_invoice_force_normal_payment'] == 'no') ) {
            $error     = EDD()->session->get( 'novalnet_invoice_guarantee_payment_error' );
            ...
            edd_set_error( 'guarantee_validation',  $error_msg);
            edd_send_back_to_checkout( '?payment-mode=novalnet_invoice' );
        }
        // Date validation for guarantee payment
       if ( isset( $edd_options['novalnet_invoice_guarantee_enable'])  && $trial_product == '0') {

            $day   = isset( $invoice_details['novalnet_invoice_day'] ) ? $invoice_details['novalnet_invoice_day'] : '';
            $month = isset( $invoice_details['novalnet_invoice_month'] ) ? $invoice_details['novalnet_invoice_month'] : '';
            $year  = isset( $invoice_details['novalnet_invoice_year'] ) ? $invoice_details['novalnet_invoice_year'] : '';

            $error      = check_guarantee_payment( $day, $month, $year, 'novalnet_invoice' );
            $date_check = EDD()->session->get( 'novalnet_invoice_dob' );

            if ( ! empty( $error ) && isset($edd_options['novalnet_invoice_force_normal_payment']) && $edd_options['novalnet_invoice_force_normal_payment'] == 'no')  {
                edd_set_error( 'guarantee_validation', $error);
                edd_send_back_to_checkout( '?payment-mode=novalnet_invoice' );
        ...
        if ( isset( $edd_options['novalnet_invoice_guarantee_enable'] ) && empty( EDD()->session->get( 'novalnet_invoice_guarantee_payment_error' ) ) && empty( EDD()->session->get( 'novalnet_invoice_guarantee_dob_payment_error' ) ) && $trial_product == '0' ) {
            $payment_name = 'novalnet_invoice_guarantee';
        }
        // Get configuration data.
        $payment_data                 = novalnet_get_merchant_data( $purchase_data, $payment_name );
        $payment_data['invoice_type'] = 'INVOICE';
        // Get customer data.
        $customer_data = novalnet_get_customer_data( $purchase_data );
        // Get system data.
        $system_data = novalnet_get_system_data();

        $payment_duration = isset( $edd_options['novalnet_invoice_due_date'] ) ? trim( $edd_options['novalnet_invoice_due_date'] ) : '';
        ...
        if (isset($edd_options['novalnet_invoice_manual_limit']) && 'authorize' === $edd_options['novalnet_invoice_manual_limit'] ) {
            if ( isset( $edd_options['novalnet_invoice_manual_check'] ) ) {
                $payment_data['amount'] = ( novalnet_digits_check( $edd_options['novalnet_invoice_manual_check'] ) ) ? $payment_data['amount'] : 0;
                if ( $payment_data['amount'] >= $edd_options['novalnet_invoice_manual_check'] ) {
                    $params['on_hold'] = 1;
                }
            } else {
                $params['on_hold'] = 1;
        ...
        if ( '100' === $parsed_response['status'] ) {
            ...
            novalnet_check_response( $parsed_response ,$trial_product);
        } else {
     ...
     */
    public static function register_novalnet_invoice_settings( $gateway_settings ) {
        // Shop backend configuration.
        $novalnet_invoice = array(
            array(
                'id'   => 'novalnet_invoice_settings',
                'name' => '<strong> <font color="#1874CD">' . __( 'Novalnet Invoice', 'edd-novalnet' ) . '</font> </strong>',
            ...
            array(
                'id'      => 'novalnet_invoice_manual_limit',
            ...
            array(
                'id'            => 'novalnet_invoice_manual_check',
                ...
                'tooltip_desc'  => __( 'When the order amount exceeds the mentioned limit, the transaction will be set on-hold till you confirm it. You can leave the field empty if you wish to process all transactions as on-hold transactions. ', 'edd-novalnet' ),
            ),
            ...
            array(
                'id'            => 'novalnet_invoice_order_callback_status',
            ...
            array(
                'id'   => 'novalnet_invoice_guarantee_settings',
                'name' => sprintf(
            ...
            array(
                'id'   => 'novalnet_invoice_guarantee_enable',
                'name' => __( 'Enable payment guarantee', 'edd-novalnet' ),
                'type' => 'checkbox',
            ),
            array(
                'id'            => 'novalnet_invoice_order_pending_status',
            ...
            array(
                'id'                => 'novalnet_invoice_guarantee_minimum_order_amount',
            ...
            array(
                'id'            => 'novalnet_invoice_force_normal_payment',
        ...
        $novalnet_invoice                     = apply_filters( 'edd_novalnet_invoice_settings', $novalnet_invoice );
        $gateway_settings['novalnet_invoice'] = $novalnet_invoice;
        return $gateway_settings;
    }
}
new Novalnet_Invoice();
