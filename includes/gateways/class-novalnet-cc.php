...
class Novalnet_Cc {
    ...
    public function novalnet_cc_script_functions() {
        ...
        // Enqueue style.
        wp_enqueue_style( 'edd-novalnet-cc-style', NOVALNET_PLUGIN_URL . 'assets/css/novalnet-checkout.css',array(), NOVALNET_VERSION );
        ...
        wp_enqueue_script( 'edd-novalnet-cc-script', NOVALNET_PLUGIN_URL . 'assets/js/novalnet-cc.min.js', array( 'jquery' ), NOVALNET_VERSION, true );

        $integrity_hash = 'sha384-GU9VTCpKpy0Z47Pre9WZJG22sXPqNw6L9tq8/4oG/dqCiWbHujkx27Wk8NVhBUV5';
        add_filter('script_loader_tag', function ($tag, $handle) use ($integrity_hash) {
            if ('edd-novalnet-cc-script' === $handle) {
                return str_replace(
                    ' src=',
    ...
    public function display_form() {
        ...
        <fieldset>
            ...
            <p>
                <label for="novalnet_cc_information">
                    <span class="edd-description"><?php echo esc_html( $information ); ?>
                    </span>
                </label>
            </p>
            <?php
        ...
        <input type="hidden" id="novalnet_cc_amount" name="novalnet_cc_amount" value="<?php echo esc_attr( edd_get_cart_total() * 100 ); ?>"/>
    ...
    public function novalnet_cc_process_payment( $purchase_data ) {
        ...
        if (isset($edd_options['novalnet_cc_manual_limit']) && 'authorize' === $edd_options['novalnet_cc_manual_limit'] ) {
            if ( isset( $edd_options['novalnet_cc_manual_check'] ) ) {
                 $manual_check_limit = novalnet_digits_check( $edd_options['novalnet_cc_manual_check'] );
                if ( $payment_data['amount'] >= $manual_check_limit ) {
                    $params['on_hold'] = 1;
                }
            } else {
                $params['on_hold'] = 1;
        ...
        if ( isset($_REQUEST['novalnet_cc_do_redirect']) && (sanitize_text_field(wp_unslash($_REQUEST['novalnet_cc_do_redirect'])) == 1 )) {
            EDD()->session->set( 'novalnet_cc_do_redirect', sanitize_text_field(wp_unslash($_REQUEST['novalnet_cc_do_redirect'])));
     ...
     */
    public static function register_novalnet_cc_settings( $gateway_settings ) {
