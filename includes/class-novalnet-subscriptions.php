...
header( 'Access-Control-Allow-Origin: ' . wp_unslash( $_SERVER['HTTP_HOST'] ) . '/*' ); // Input var okay.
...
class Novalnet_Subscriptions {
    ...
    public function process_novalnet_payment_method_update( $user_id, $subscription_id, $verified ) {
        ...
        if ( 1 !== $verified || empty( $subscription->id ) || empty( $subscriber->id ) || ( $user_id != $subscriber->user_id ) ) {
            wp_die( __( 'Invalid subscription id', 'edd-novalnet' ) );
        }
        if ( novalnet_check_string( $subscription->gateway ) ) {
            ...
            $params        = array(
                'amount'             => 0,
                'gender'             => 'u',
                'customer_no'        => $subscription->customer_id,
            ...
            if ( ( 'novalnet_cc' === $subscription->gateway && 1 == $request['novalnet_cc_do_redirect'] )|| 'novalnet_paypal' === $subscription->gateway ) {
                ...
                $subs_update = array( 'update_payment' => true,
    ...
    public function novalnet_subscription_enqueue_scripts() {
        ...
        wp_enqueue_script( 'edd-novalnet-subscription-script', NOVALNET_PLUGIN_URL . 'assets/js/novalnet-subscription.js', array( 'jquery' ), NOVALNET_VERSION, true );
        wp_enqueue_style( 'edd-novalnet-subscription-script', NOVALNET_PLUGIN_URL . 'assets/css/novalnet-checkout.css', array(), NOVALNET_VERSION, true );
        $params = array(
    ...
    public function novalnet_subscription_cancel_form() {
        ...
        $form .= '</select><br/><input type="submit" class="button novalnet_cancel" style="background:#0085ba;border-color:#0073aa #006799 #006799;color:#fff;" onclick="return novalnet_hide_button(this);" id="novalnet_cancel" value=' . __( 'Cancel', 'novalnet' ) . '></form>';
        return $form;
    }
    ...
    public function novalnet_process_cancellation( $data ) {
        ...
        if ( 'cancelled' !== $subscription->status ) {
            ...
            if ( '100' === $cancel_response['status'] ) {
                ...
                edd_update_payment_status( $subscription->parent_payment_id, 'abandoned' );
                $wpdb->update(
            ...
            $result         = $wpdb->get_results( $wpdb->prepare( "SELECT post_excerpt FROM {$wpdb->posts} where ID =%s", $subscription->parent_payment_id ) ); // db call ok; no-cache ok.
            $exist_comments = $result[0]->post_excerpt;
            $nn_order_notes = array(
                'ID'           => $subscription->parent_payment_id,
                'post_excerpt' => $exist_comments . $cancel_msg,
            );
    ...
    public function novalnet_can_cancel( $can_cancel, $subscriptions ) {
        ...
        $get_times_billed      = ( $edd_recurring_version < '2.6' ) ? $subscriptions->get_total_payments() : $subscriptions->get_times_billed();
