...
function novalnet_callback_api_process() {
    ...
    if ( ! empty( $order_reference['order_no'] ) ) {
        ...
        if ( ! in_array( $vendor_script->ary_request_params['payment_type'], $vendor_script->ary_payment_groups[ $order_reference['payment_type'] ], true ) ) {
        ...
        $vendor_script->perform_subscription_stop_request( $request_param, $order_reference );
        ...
        if ( ! empty( $nn_order_id ) ) {
            ...
            if ( !empty( $post_details) && $post_details->post_type == 'edd_payment' ) {
                $payment = get_novalnet_payment( $nn_order_id );
                if ( $nn_order_id == $post_details->ID && novalnet_check_string($payment) ) {
                    $vendor_script->handle_communication_failure( $payment );
                } else {
                    $vendor_script->debug_error( 'Order number is not valid' );
                } 
            } 
        }
...
class Novalnet_Callback_Api {
    ...
    public $ary_request_params = array();
      ...
      * @var array
      */
    public $ary_payments = array( 'CREDITCARD', 'INVOICE_START', 'DIRECT_DEBIT_SEPA', 'GUARANTEED_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'PAYPAL', 'ONLINE_TRANSFER', 'IDEAL', 'EPS', 'GIROPAY', 'PRZELEWY24', 'CASHPAYMENT' );
      ...
      * @var array
      */
    public $ary_chargebacks = array( 'GUARANTEED_INVOICE_BOOKBACK', 'GUARANTEED_SEPA_BOOKBACK', 'RETURN_DEBIT_SEPA', 'REVERSAL', 'CREDITCARD_BOOKBACK', 'CREDITCARD_CHARGEBACK', 'REFUND_BY_BANK_TRANSFER_EU', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND', 'REFUND_BY_BANK_TRANSFER_EU', 'CASHPAYMENT_REFUND' );
    ...
    public function check_ip_address() {
        ...
        $get_ip_address = get_remote_address();
    ...
    public function handle_communication_failure( $payment ) {
        ...
        $key = ( isset( $this->ary_request_params['key'] ) && !empty( $this->ary_request_params['key'] ) ) ? $this->ary_request_params['key'] : $this->ary_request_params['payment_id'];
        
        $amount = str_replace('.', ',', sprintf('%.2f', $this->ary_request_params['amount'] / 100 ) );

        if ( '100' === $this->ary_request_params ['status'] ) {

            update_post_meta( $this->ary_request_params['order_no'], '_edd_payment_gateway', $payment );
            update_post_meta( $this->ary_request_params['order_no'], '_nn_order_tid', $this->ary_request_params['tid'] );
            ...
            if ( in_array( $payment, $invoice_payments, true ) || ( 'novalnet_paypal' === $payment && ( '90' === $this->ary_request_params['tid_status'] || '85' === $this->ary_request_params['tid_status'] ) ) || ( 'novalnet_przelewy24' === $payment && '86' === $this->ary_request_params['tid_status'] ) || ( 'novalnet_cashpayment' === $payment ) ) {
                update_post_meta( $this->ary_request_params['order_no'], '_nn_callback_amount', 0 );
            } else {
                // Set the purchase to complete.
                update_post_meta( $this->ary_request_params['order_no'], '_nn_callback_amount', get_post_meta( $this->ary_request_params['order_no'], '_edd_payment_total', true ) * 100 );
            }
            ...
            if ( in_array( $payment, $invoice_payments, true ) ) {
                $novalnet_comments = novalnet_get_invoice_comments( $this->ary_request_params, $payment, $amount, $novalnet_comments, true, true );
            }
    ...
    public function recurring_order_creation( $subscription_id, $request_param ) {
        global $wpdb, $test_mode, $edd_options;
        $recurring_order       = '';
        $subscription          = new EDD_Subscription( $subscription_id );
        $recurring_method      = get_post_meta( $subscription->parent_payment_id, '_edd_payment_gateway', true );
        ...
        $get_times_billed      = ( $edd_recurring_version < '2.6' ) ? $subscription->get_total_payments() : $subscription->get_times_billed();
        ...
        if ( ( ( ( $get_times_billed < $subscription->bill_times ) && 0 !== $subscription->bill_times ) || '0' === $subscription->bill_times ) && empty( $trans_details['termination_reason'] ) ) {
            ...
            update_post_meta( $recurring_order, '_nn_order_tid', $request_param['tid'] );
            update_post_meta( $recurring_order, '_edd_payment_total', $request_param['amount'] / 100 );
            update_post_meta( $recurring_order, '_edd_payment_transaction_id', $recurring_order );
            ...
            update_post_meta( $request_param['tid'], '_order_total', $request_param['amount'] / 100 );
            ...
            if ( 'INVOICE_START' === $request_param['payment_type'] ) {
                $request_param['order_no'] = $recurring_order;
                $novalnet_comments         = novalnet_get_invoice_comments( $request_param, $recurring_method, sprintf( '%0.2f', ( $request_param['amount'] / 100 ) ), $novalnet_comments, true, true );
            ...
            $callback_comments = PHP_EOL . sprintf( __( 'Novalnet Callback Script executed successfully for the TID: %1$s with amount %2$s on %3$s. Please refer PAID transaction in our Novalnet Admin Portal with the TID: %4$s', 'edd-novalnet' ), $request_param['shop_tid'], edd_currency_filter( edd_format_amount( $request_param ['amount'] / 100 ) ), date_i18n( get_option( 'date_format' ), strtotime( gmdate( 'Y-m-d' ) ) ), $request_param['tid'] ) . PHP_EOL;
            update_post_meta( $recurring_order, '_nn_invoice_comments', $novalnet_comments . PHP_EOL . $next_payment_date );
            update_post_meta( $recurring_order, '_payment_method', $recurring_method );
            update_post_meta( $recurring_order, '_nn_version', NOVALNET_VERSION );
    ...
    public function perform_subscription_stop_request( $request_param ) {
        ...
        $subs_billing_check = ( '1' === $request_param['subs_billing'] || '0' === $request_param['subs_billing'] );
        
        if ( ( $subs_billing_check && in_array( $request_param ['payment_type'], array( 'SUBSCRIPTION_STOP', 'TRANSACTION_CANCELLATION' ), true ) ) || ( $subs_billing_check && '100' !== $request_param['status'] ) ) {

            // subscription cancel reason
            $subs_cancel_reason = ! empty( $request_param['termination_reason'] ) ? $request_param['termination_reason'] : ( isset( $request_param['status_desc'] ) ? $request_param['status_desc'] : ( isset( $request_param['status_message'] ) ? $request_param['status_message'] : ( isset( $request_param['status_text'] ) ? $request_param['status_text'] : '' ) ) );
            
            /* translators: %1$s: tid, %2$s: date, %3$s: reason */
            $request_param['callback_comments'] = PHP_EOL . sprintf( __( 'Novalnet callback script received. Subscription has been stopped for the TID: %1$s on %2$s. Subscription has been canceled due to: %3$s', 'edd-novalnet' ), $request_param['signup_tid'], date_i18n( get_option( 'date_format' ), strtotime( gmdate( 'Y-m-d' ) ) ), $subs_cancel_reason ) . PHP_EOL;
            ...
            $wpdb->update(
                $wpdb->prefix . 'novalnet_subscription_details',
                array(
                    'termination_reason' => $subs_cancel_reason,
                    'termination_at'     => gmdate( 'Y-m-d H:i:s' ),
                ),
                array(
                    'order_no' => $order_reference['order_no'],
    ...
    public function perform_subscription_reactivation_request( $request_param ) {
        ...
        if ( 'SUBSCRIPTION_REACTIVATE' === $request_param['payment_type'] && '100' === $request_param['status'] || ( '100' !== $request_param['status'] && ! empty( $request_param['subs_billing'] ) ) ) {
            ...
            $request_param['callback_comments'] = PHP_EOL . sprintf( __( 'Subscription has been reactivated for the TID: %1$s on %2$s. Next charging date: %3$s', 'edd-novalnet' ), $request_param['signup_tid'], date_i18n( get_option( 'date_format' ), strtotime( gmdate( 'Y-m-d' ) ) ), date_i18n( get_option( 'date_format' ), strtotime( $next_payment_date ) ) ) . PHP_EOL;
            $wpdb->update(
        ...
        if ( 0 === $payment_level ) {
            ...
            if ( ! empty( $request_param['subs_billing'] ) && '1' === $request_param['subs_billing'] ) {
                ...
                if ( in_array( $request_param['payment_type'], array( 'INVOICE_START', 'GUARANTEED_INVOICE' ), true ) && in_array( $order_reference['gateway_status'], array( '75', '91' ), true ) && in_array( $request_param['tid_status'], array( '91', '100' ) ) ) {
                    ...
                    $request_param['message'] .= novalnet_get_invoice_comments( $request_param, $payment_gateway, $org_amount, '', true, true );
                }
                $wpdb->update(
                    $wpdb->prefix . 'novalnet_transaction_detail',
                    array(
                ...
                if ( $callback_amount < $org_amount ) {
                    update_post_meta( $order_reference['order_no'], '_nn_callback_amount', $org_amount );
                    /* translators: %1$s: tid, %2$s: amount, %3$s: date */
                    $request_param['message'] = PHP_EOL . sprintf( __( 'Novalnet Callback Script executed successfully for the TID: %1$s with amount %2$s on %3$s.', 'edd-novalnet' ), $request_param['shop_tid'], edd_currency_filter( edd_format_amount( $request_param ['amount'] / 100 ) ), gmdate( 'Y-m-d H:i:s' ) ) . PHP_EOL;
                    ...
                    $this->debug_error( 'Novalnet callback received. Order already Paid.' );
                }
                ...
                if ( '86' === $request_param['tid_status'] ) {
                    $this->debug_error( 'Novalnet Callbackscript received. Payment type ( ' . $request_param['payment_type'] . ' ) is not applicable for this process!' );
                }
    ...
    public function first_level_process( $request_param ) {
        global $order_reference, $payment_level;

        if ( 1 === $payment_level && '100' === $request_param['status'] && '100' === $request_param['tid_status'] ) {
            
            if ( in_array( $request_param['payment_type'], array( 'CREDITCARD_BOOKBACK', 'GUARANTEED_INVOICE_BOOKBACK', 'GUARANTEED_SEPA_BOOKBACK', 'PAYPAL_BOOKBACK', 'REFUND_BY_BANK_TRANSFER_EU', 'PRZELEWY24_REFUND', 'CASHPAYMENT_REFUND' ), true ) ) {
                /* translators: %1$s: parent tid, %2$s: amount, %3$s: tid  */
                $request_param['message'] = PHP_EOL . sprintf( __( 'Refund has been initiated for the TID: %1$s with the amount %2$s. New TID: %3$s.', 'edd-novalnet' ), $request_param['shop_tid'], edd_currency_filter( edd_format_amount( $request_param ['amount'] / 100 ) ), $request_param['tid'] ) . PHP_EOL;
            ...
            edd_update_payment_status( $order_reference['order_no'], 'refunded' );
            $this->update_comments( $order_reference, $request_param['message'] );
            $this->send_notify_mail( array( 'comments' => $request_param['message'] ) );
        }
    }
    ...
    public function second_level_process( $request_param ) {
        ...
        if ( 2 === $payment_level && '100' === $request_param['status'] && '100' === $request_param['tid_status'] ) {
            ...
            if ( in_array( $request_param['payment_type'], array( 'INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'ONLINE_TRANSFER_CREDIT' ) ) ) {
                if ( $callback_amount < $org_amount ) {
                    /* translators: %1$s: parent tid, %2$s: amount, %3$s: date, %4$s: tid  */
                    $request_param['message'] = PHP_EOL . sprintf( __( 'Novalnet Callback Script executed successfully for the TID: %1$s with amount %2$s on %3$s. Please refer PAID transaction in our Novalnet Admin Portal with the TID: %4$s', 'edd-novalnet' ), $request_param['shop_tid'], edd_currency_filter( edd_format_amount( $request_param ['amount'] / 100 ) ), gmdate( 'Y-m-d H:i:s' ), $request_param['tid'] ) . PHP_EOL;
                    
                    if ( 'ONLINE_TRANSFER_CREDIT' === $request_param['payment_type'] ) {
                        $request_param['message'] = PHP_EOL . sprintf( __( 'Novalnet Callback Script executed successfully for the TID: %1$s with amount %2$s on %3$s. Please refer PAID transaction in our Novalnet Admin Portal with the TID: %4$s. %5$sPlease verify received amount and TID details, and update the order status accordingly. ', 'edd-novalnet' ), $request_param['shop_tid'], edd_currency_filter( edd_format_amount( $request_param ['amount'] / 100 ) ), gmdate( 'Y-m-d H:i:s' ), $request_param['tid'],PHP_EOL ) . PHP_EOL;
                    }
                    
                    update_post_meta( $order_reference['order_no'], '_nn_callback_amount', $sum_amount );
                    $wpdb->update(
                        $wpdb->prefix . 'novalnet_transaction_detail',
                        array(
                    ...
                    if ( $sum_amount >= $org_amount ) {
                        EDD()->session->set( 'novalnet_transaction_comments', $request_param['message'] );
                        edd_update_payment_status( $order_reference['order_no'], $edd_options[ $payment_gateway . '_order_callback_status' ] );
                    }
                    $this->send_notify_mail( array( 'comments' => $request_param['message'] ) );
                }// End if.
            ...
            $this->debug_error( 'Novalnet callback received. Payment type ( ' . $request_param['payment_type'] . ') is not applicable for this process!' );
        }// End if.
    }
    ...
    public function get_payment_data( &$request_param, $order_reference ) {
        ...
        $sum_amount                = ( isset( $request_param['amount'] ) && ! empty( $request_param['amount'] ) ? $request_param['amount'] : 0 ) + $callback_amount;
        $org_amount                = round( get_post_meta( $order_reference, '_edd_payment_total', true ), 2 ) * 100;
