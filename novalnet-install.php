...
function novalnet_activation_process()
{
    ...
    if (! get_option('novalnet_db_version') || NOVALNET_VERSION !== get_option('novalnet_db_version')) {
        ...
        refund_amount int(11) DEFAULT '0' COMMENT 'Refund amount in cents',
        ...
        $result = $wpdb->get_results("SHOW COLUMNS FROM {$wpdb->prefix}novalnet_transaction_detail LIKE 'refund_amount'");
        if (empty($result)) {
            $wpdb->query("ALTER TABLE {$wpdb->prefix}novalnet_transaction_detail ADD COLUMN refund_amount int(11) DEFAULT 0");
        }
        ...
        if (! get_option('novalnet_db_version')) {
            ...
            if (version_compare(get_option('novalnet_db_version'), '2.4.0',  '<=')) {
