...
<fieldset>
	...
	if ( isset( $edd_options['novalnet_invoice_guarantee_enable'] ) && 0 !== get_current_user_id() && !$update_payment ) {
		...
		<p>
			...
			<span class="edd-description" style="color:red;font-size:90%;">
			<?php
				$invoice_error_msg = novalnet_guarantee_payment_validation( $user_data, $payment_name );
			if ( $invoice_error_msg ) {
				/* translators: %s: error message */
				$invoice_error_msg = sprintf( __( 'The payment cannot be processed, because the basic requirements for the payment guarantee are not met <br>  %s   ', 'edd-novalnet' ), $invoice_error_msg );
				echo __( $invoice_error_msg, 'edd-novalnet' );
			}
			?>
			</span>
		</label>
	...
	<?php
	if (  isset( $edd_options['novalnet_invoice_guarantee_enable'] ) && ( empty( EDD()->session->get( 'novalnet_invoice_guarantee_payment_error' ) )  || ( 0 === get_current_user_id() ) ) && !$update_payment ) {
